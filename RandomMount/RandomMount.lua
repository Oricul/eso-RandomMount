...
function RM_Object:StructureAndFix()
	...
	local data = {self.account.mounts, self.account.pets, self.account.skins, self.settings.mounts, self.settings.pets, self.settings.skins}
...
function RM_Object:GetMountOptions()
	...
	for id,obj in pairs(self.account.mounts) do
		mounts[#mounts+1] = {
			...
			setFunc = function(value) self[self:GetKey()].mounts[id].use = value end,
			disabled = function()
				local key = self:GetKey()
				if self[key].mounts[id] then
...
function RM_Object:GetPetOptions()
	...
	for id,obj in pairs(self.account.pets) do
		pets[#pets+1] = {
			...
			setFunc = function(value) self[self:GetKey()].pets[id].use = value end,
			disabled = function()
				local key = self:GetKey()
				if self[key].pets[id] then
...
function RM_Object:GetSkinOptions()
	...
	for id,obj in pairs(self.account.skins) do
		skins[#skins+1] = {
			...
			setFunc = function(value) self[self:GetKey()].skins[id].use = value end,
			disabled = function()
				local key = self:GetKey()
				if self[key].skins[id] then
	...
	local optionsData = {
		{
			type = "checkbox",
			name = RM_OP_ACCOUNT,
			getFunc = function() return self.settings.useAccountWide end,
			setFunc = function(value) self.settings.useAccountWide = value end,
			default = defaults.settings.useAccountWide
		},{
			type = "slider",
			name = RM_OP_DELAY,
			tooltip = RM_OP_DELAY_TT,
			getFunc = function() return self[self:GetKey()].delay end,
			setFunc = function(value) self[self:GetKey()].delay = value end,
			min = 1,
			max = 7,
			step = 1,
			...
			getFunc = function() return self[self:GetKey()].mount.enable end,
			setFunc = function(value) self[self:GetKey()].mount.enable = value end,
			default = defaults[self:GetKey()].mount.enable
		},{
			type = "checkbox",
			name = RM_OP_MOUNTS_ENABLE_PVP,
			getFunc = function() return self[self:GetKey()].mount.enable_pvp end,
			setFunc = function(value) self[self:GetKey()].mount.enable_pvp = value end,
			disabled = function() return not self[self:GetKey()].mount.enable end,
			default = function() return defaults[self:GetKey()].mount.enable_pvp end
		},{
			type = "checkbox",
			name = RM_OP_ACTIVATE_ZONE,
			getFunc = function() return self[self:GetKey()].mount.zone end,
			setFunc = function(value) self[self:GetKey()].mount.zone = value end,
			disabled = function() return not self[self:GetKey()].mount.enable end,
			default = defaults[self:GetKey()].mount.zone
		},{
			type = "checkbox",
			name = RM_OP_ACTIVATE_DISMOUNT,
			getFunc = function() return self[self:GetKey()].mount.dismount end,
			setFunc = function(value) self[self:GetKey()].mount.dismount = value end,
			disabled = function() return not self[self:GetKey()].mount.enable end,
			default = defaults[self:GetKey()].mount.dismount
		},{
			type = "checkbox",
			name = RM_OP_MOUNTS_MULTI_GROUPED,
			getFunc = function() return self[self:GetKey()].mount.preferMultiWhenGrouped end,
			setFunc = function(value) self[self:GetKey()].mount.preferMultiWhenGrouped = value end,
			disabled = function() return not self[self:GetKey()].mount.enable end,
			default = defaults[self:GetKey()].mount.preferMultiWhenGrouped
		},{
			type = "checkbox",
			name = RM_OP_MOUNTS_MULTI_COMPANION,
			getFunc = function() return self[self:GetKey()].mount.preferMultiWhenCompanionOut end,
			setFunc = function(value) self[self:GetKey()].mount.preferMultiWhenCompanionOut = value end,
			disabled = function() return not self[self:GetKey()].mount.enable end,
			...
			disabledLabel = function() return not self[self:GetKey()].mount.enable end,
			...
			getFunc = function() return self[self:GetKey()].pet.enable end,
			setFunc = function(value) self[self:GetKey()].pet.enable = value end,
			default = defaults[self:GetKey()].pet.enable
		},{
			type = "checkbox",
			name = RM_OP_ACTIVATE_ZONE,
			getFunc = function() return self[self:GetKey()].pet.zone end,
			setFunc = function(value) self[self:GetKey()].pet.zone = value end,
			disabled = function() return not self[self:GetKey()].pet.enable end,
			default = defaults[self:GetKey()].pet.zone
		},{
			type = "checkbox",
			name = RM_OP_ACTIVATE_DISMOUNT,
			getFunc = function() return self[self:GetKey()].pet.dismount end,
			setFunc = function(value) self[self:GetKey()].pet.dismount = value end,
			disabled = function() return not self[self:GetKey()].pet.enable end,
			...
			disabledLabel = function() return not self[self:GetKey()].pet.enable end,
			...
			getFunc = function() return self[self:GetKey()].skin.enable end,
			setFunc = function(value) self[self:GetKey()].skin.enable = value end,
			default = defaults[self:GetKey()].skin.enable
		},{
			type = "checkbox",
			name = RM_OP_ACTIVATE_ZONE,
			getFunc = function() return self[self:GetKey()].skin.zone end,
			setFunc = function(value) self[self:GetKey()].skin.zone = value end,
			disabled = function() return not self[self:GetKey()].skin.enable end,
			default = defaults[self:GetKey()].skin.zone
		},{
			type = "checkbox",
			name = RM_OP_ACTIVATE_DISMOUNT,
			getFunc = function() return self[self:GetKey()].skin.dismount end,
			setFunc = function(value) self[self:GetKey()].skin.dismount = value end,
			disabled = function() return not self[self:GetKey()].skin.enable end,
			default = defaults[self:GetKey()].skin.dismount
		},{
			type = "checkbox",
			name = RM_OP_ALLOW_DEFAULT,
			getFunc = function() return self[self:GetKey()].skin.allowDefault end,
			setFunc = function(value) self[self:GetKey()].skin.allowDefault = value end,
			disabled = function () return not self[self:GetKey()].skin.enable end,
			...
			disabledLabel = function() return not self[self:GetKey()].skin.enable end,
				...
				for collectibleIndex=1, numCollectibles do
					...
					if collectibleData:IsUnlocked() then
						...
						if categoryType == COLLECTIBLE_CATEGORY_TYPE_MOUNT then
							...
							self.account.mounts[id] = self.account.mounts[id] or { use = true }
							self.settings.mounts[id] = self.settings.mounts[id] or { use = true }
						end
						if categoryType == COLLECTIBLE_CATEGORY_TYPE_VANITY_PET then
							local id = collectibleData:GetId()
							self.account.pets[id] = self.account.pets[id] or { use = true }
							self.settings.pets[id] = self.settings.pets[id] or { use = true }
						end
						if categoryType == COLLECTIBLE_CATEGORY_TYPE_SKIN then
							local id = collectibleData:GetId()
							self.account.skins[id] = self.account.skins[id] or { use = true }
							self.settings.skins[id] = self.settings.skins[id] or { use = true }
						end
					else
...
function RM_Object:OnMountedStateChanged(isMounted)
	if self[self:GetKey()].mount.dismount then
		if not isMounted then
			zo_callLater(function() self:SummonMount() end, 1200)
		end
	end
	if self[self:GetKey()].pet.dismount then
		if not isMounted then
			zo_callLater(function() self:SummonPet() end, 1200)
		end
	end
	if self[self:GetKey()].skin.dismount then
		if not isMounted then
			zo_callLater(function() self:ChangeSkin() end, 1200)
		end
	end
end
...
-- @Origami: ZO_COLLECTION_DATA_MANAGER:IsCollectibleAvailableToCompanion() - Line 481 - https://github.com/esoui/esoui/blob/148bf16c4c457ca9d75e41e7045e59de624b1ae7/esoui/ingame/collections/collectibledatamanager.lua
-- @Origami: UseCollectible(self.collectibleId, actorCategory) - Line 510 - https://github.com/esoui/esoui/blob/148bf16c4c457ca9d75e41e7045e59de624b1ae7/esoui/ingame/collections/collectibledatamanager.lua
-- @Origami: actorCategory may be UnitTag
-- @Origami: UseCollectible(id,"companion")
...
function RM_Object:IsMultiRider(id)
	...
	local subCategoryName = ZO_COLLECTIBLE_DATA_MANAGER:GetCategoryDataByIndicies(categoryData.categoryIndex,categoryData.subcategoryIndex).name
		...
		if #useable > 0 then
			...
			if timeBetweenChange > delay then
				if (not self:IsAssistantOut()) and (not IsCollectibleActive(newSkin)) and (not IsUnitInCombat("player")) then
					if newSkin == -1 then
						UseCollectible(self[k].currentSkinId)
						self[k].currentSkinId = newSkin
...
function RM_Object:SummonPet()
	...
	if self[k].pet.enable then
		...
		if #useable > 0 then
			...
			if ( #useable > 1 ) then
				while ( newPet == self[k].currentPetId )
				do
			...
			local delay = self[self:GetKey()].delay
			if timeBetweenChange > delay then
				if (not self:IsAssistantOut()) and (not IsCollectibleActive(newPet)) and IsCollectibleUsable(newPet) and (not IsUnitInCombat("player")) then
					self[k].currentPetId = newPet
...
-- @Origami: Need to research this section. It's probably not needed.
function RM_Object:Test()
	d("RandomMount DEBUG: Test")
	self:OnCollectionUpdated()

	-- @Weolo: remove ?
	self.account.mounts[291] = nil
	self.settings.mounts[291] = nil
	if self.account.mounts[291] then
		d("RandomMount DEBUG: ? WAS NOT removed")
	else
		d("RandomMount DEBUG: ? WAS removed")
	end

	-- @Weolo: GetData() brings the bear back as it is a mount you own?
	if self.settingsCreated then
		-- @Weolo: self:GetData()
		self.panel:RefreshPanel()
		d("RandomMount DEBUG: What did that do? Did it disable the setting box?")
	else
		d("RandomMount DEBUG: What did that do? Because the panel was not loaded, if you now go to the settings the bear should not be there?")
	end
end

-- @Origami: Need to research this section. It's probably not needed.
function RM_Object:TestAdd()
	-- @Weolo: /script RandomMount:TestAdd()
	-- @Weolo: Add a new mount
	local id = 291
	local name = ZO_CachedStrFormat(SI_COLLECTIBLE_NAME_FORMATTER, GetCollectibleName(id))
	self.account.mounts[id] = { use = true }
	self.settings.mounts[id] = { use = true }
	if self.account.mounts[id] then
		d(sf("RandomMount DEBUG: %s WAS added", name))
	else
		d(sf("RandomMount DEBUG: %s WAS NOT added", name))
	end
end

-- @Origami: Need to research this section. It's probably not needed.
function RM_Object:TestRemove()
	-- @Weolo: /script RandomMount:TestRemove()
	-- @Weolo: Remove a mount
	local id = 291
	local name = ZO_CachedStrFormat(SI_COLLECTIBLE_NAME_FORMATTER, GetCollectibleName(id))
	self.account.mounts[id] = nil
	self.settings.mounts[id] = nil
	if self.account.mounts[id] then
		d(sf("RandomMount DEBUG: %s WAS NOT removed", name))
	else
		d(sf("RandomMount DEBUG: %s WAS removed", name))
	end
end
...
function RM_Object:OnPlayerActivated()
	if self.player_activated then return end -- @Weolo: Only the first time
	...
	EVENT_MANAGER:RegisterForEvent(self.ADDON_NAME, EVENT_ZONE_CHANGED, function(_, ...) self:OnZone(...) end)
	EVENT_MANAGER:RegisterForEvent(self.ADDON_NAME, EVENT_PLAYER_ACTIVATED, function() zo_callLater(function() self:OnZone() end, 2000) end)
	EVENT_MANAGER:RegisterForEvent(self.ADDON_NAME, EVENT_COLLECTION_UPDATED, function(...) self:OnCollectionUpdated() end) -- @Weolo: Full refresh
	EVENT_MANAGER:RegisterForEvent(self.ADDON_NAME, EVENT_COLLECTIBLES_UNLOCK_STATE_CHANGED, function(...) self:OnCollectionUpdated() end) -- @Weolo: Need to check more
	EVENT_MANAGER:RegisterForEvent(self.ADDON_NAME, EVENT_MOUNTED_STATE_CHANGED, function(_, isMounted) self:OnMountedStateChanged(isMounted) end)
	CALLBACK_MANAGER:RegisterCallback("LAM-PanelControlsCreated", function(panel) self:OnSettingsControlsCreated(panel) end)
end
...
EVENT_MANAGER:RegisterForEvent(RandomMount.ADDON_NAME, EVENT_ADD_ON_LOADED, function(_, addonName) RandomMount:OnLoaded(addonName) end)